...
import { parseSmartCarDetails } from "@/lib/utils/parseSmartCarDetails";
"use client";
import React, { useState, useEffect, useRef } from "react";
import Image from "next/image";
import { createClient } from "@/lib/supabase/client";
...
export default function DealerAddCarPage() {
  ...
  useEffect(() => {
    const result = parseSmartCarDetails(rawText);
    setParsed(result);
  }, [rawText]);
  ...
  const canPublish =
    !!parsed.title && !!parsed.price && images.length > 0 && parsed.confidence >= 60 && !loading;
      ...
      >
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {Object.entries(parsed)
            .filter(([k, v]) =>
              ["year", "make", "model", "version", "price", "km", "fuel", "transmission", "owner"].includes(k) && v
            )
            .map(([k, v]) => (
              <span
                key={k}
